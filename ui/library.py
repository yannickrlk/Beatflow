...
class SampleRow(ctk.CTkFrame):
    ...
    def set_playing(self, is_playing, sync_active: bool = False):
        ...
        if is_playing:
            ...
            self.set_progress(0)
            # Show sync indicator if sync is active and sample has BPM
            if sync_active and self.sync_indicator:
                self.show_sync_indicator()
            else:
    ...
    def show_sync_indicator(self):
        """Show the sync indicator next to BPM."""
        if self.sync_indicator:
            try:
                self.sync_indicator.pack(side="left", padx=(2, SPACING['sm']))
            except Exception:
                pass

    def hide_sync_indicator(self):
        """Hide the sync indicator."""
        if self.sync_indicator:
            try:
                self.sync_indicator.pack_forget()
            except Exception:
                pass
    ...
    def _create_bpm_label(self, bpm_value: str, is_detected: bool):
        ...
        # Sync indicator (hidden by default, shown when sync is active)
...
class SampleList(ctk.CTkFrame):
    ...
    def __init__(self, master, on_play_request=None, on_edit_request=None,
        ...
        self.is_matching_view = False  # Track if showing match results
        ...
        self.match_source_sample = None  # The sample we're finding matches for
    ...
    def _get_current_breadcrumb_text(self):
        ...
        if self.is_favorites_view:
            return "\u2605 Favorites"
        elif self.is_collection_view:
            db = get_database()
            collection = db.get_collection(self.current_collection_id)
            return f"\u25A1 {collection['name']}" if collection else "Collection"
